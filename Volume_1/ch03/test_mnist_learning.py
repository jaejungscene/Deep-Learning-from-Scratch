...
size_of_predict = len(x)
print('the number of predicted data :',size_of_predict)
...
# ## size_of_predict만큼의 사진을 한번에 처리 -> "batch 처리"
# y = predict(network, x[:size_of_predict])
# p = np.argmax(y, axis=1)  # 확률이 가장 높은 원소의 인덱스를 얻는다.(array에서 가장 높은 값을 가지는 인덱스를 반환한다.)
# accuracy_cnt = len(p[t[:size_of_predict] == p])
# print("Accuracy:", str(float(accuracy_cnt)/len(x[:size_of_predict]))) # Accuracy: 0.9352  <-- 확률
